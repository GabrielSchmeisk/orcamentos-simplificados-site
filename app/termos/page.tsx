      ...
      <section className="legal-section">
        <div className="container">
          <aside className="legal-index">
            ...
            {["aceite", "licenca", "responsabilidades", "dados", "atualizacoes", "disponibilidade", "contato"].map((id) => (
              <a key={id} href={`#${id}`}>
                {id.replace("dados", "Dados e privacidade").replace("aceite", "Aceite").replace("licenca", "Licença").replace("responsabilidades", "Responsabilidades").replace("atualizacoes", "Atualizações").replace("disponibilidade", "Disponibilidade").replace("contato", "Contato")}
              </a>
            ))}
          </aside>
          <article className="legal-copy">
            ...
            <section id="contato"><h2>7. Contato</h2><p>Para suporte, use o canal configurado no aplicativo. Ao relatar um problema, não envie senhas, bancos completos ou documentos com dados pessoais sem orientação específica e canal protegido.</p></section>
          </article>
        </div>
      </section>
    </main>
